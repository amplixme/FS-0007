...
const Profile = ({ profile, onClickEdit }) => {
  ...
  return (
    <section className="max-w-[900px] mx-auto mb-12">
      <div className="bg-surface-container-lowest rounded-[16px] p-8 md:p-12 shadow-[0_20px_40px_rgba(17,24,39,0.05)] flex flex-col md:flex-row items-center md:items-start gap-8 md:gap-10 transition-all duration-300">
        {/* <!-- Avatar --> */}
        ...
        <div className="flex-grow text-center md:text-left space-y-4">
          ...
          <div className="flex flex-wrap items-center justify-center md:justify-start gap-4 text-sm font-medium text-slate-500">
            <span className="flex items-center gap-1.5">
              <span className="material-symbols-outlined text-[18px]" data-icon="article">
                article
              </span>
              {profile._count.posts == 1
                ? `${profile._count.posts} Publicación`
                : `${profile._count.posts} Publicaciones`}
            </span>
            <span className="w-1 h-1 bg-slate-300 rounded-full"></span>
            <span className="flex items-center gap-1.5">
              <span className="material-symbols-outlined text-[18px]" data-icon="calendar_today">
                calendar_today
              </span>
              Miembro desde{" "}
