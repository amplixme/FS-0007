            ...
            <div className="flex gap-2 overflow-x-auto pb-2 md:hidden -mx-4 px-4">
                {items.map((cat) => (
                    <button
                        ...
                        className={`shrink-0 px-4 py-2 rounded-full text-sm font-medium whitespace-nowrap transition-colors ${activeSlug === cat.slug
                    ...
                    {items.map((cat) => (
                        <li key={cat.id}>
                            <button
                                ...
                                onClick={() => onChange(cat.slug)}
                                aria-current={activeSlug === cat.slug ? "true" : undefined}
                                className={`w-full text-left px-3 py-2 rounded-md text-sm transition-colors ${activeSlug === cat.slug
